// Update generateCode.ts to work without requiring type parameter names
...
export function generateFunctionCode(
  ...
  const typeParams = func.function.typeParameters
    .map((tp: any, i: number) => {
      const name =
        typeParameterNames?.[i] ??
        (func.function.typeParameters.length === 1 ? "T" : `T${i}`);
  ...
  const insideCodeString =
    func.insideCode && Array.isArray(func.insideCode)
      ? func.insideCode
          .map((code: any) => {
            if (code.return && code.return.length > 0) {
              return `    let ${
                code.return.length === 1
                  ? `${code.parameters.map((p: any) => p).join(", ")}`
                  : `(${code.parameters.map((p: any) => p).join(", ")}`
              }${code.return.length === 1 ? "" : ")"} = ${
                code.functionName
              }(${code.parameters.map((p: any) => p).join(", ")})`;
...
${insideCodeString || "    // Function implementation"}
}`;
}
...
export function generateMoveCode({
  ...
  moduleName?: string;
  address?: string;
  structTypeParameterNames?: Record<string, string[]>;
  functionTypeParameterNames?: Record<string, string[]>;
}): string {
  ...
  const structSection = Object.entries(structs)
    .map(([name, struct]) =>
      generateStructCode(name, struct, structTypeParameterNames?.[name])
    )
    .join("\n\n");
  ...
  const functionSection = Object.entries(functions)
    .map(([name, func]) =>
      generateFunctionCode(
        name,
        func as SuiMoveFunction,
        functionTypeParameterNames?.[name]
      )
    )
    .join("\n\n");
